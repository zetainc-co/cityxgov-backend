...
export class EnfoquePoblacionalService {
    ...
    async create(createRequest: EnfoquePoblacionalRequest): Promise<EnfoquePoblacionalResponse> {
        try {
            const { data, error } = await this.supabaseService.clientAdmin
                .from('enfoque_poblacional')
    ...
    async update(id: number, updateRequest: EnfoquePoblacionalRequest): Promise<EnfoquePoblacionalResponse> {
        try {
            ...
            if (existingError) {
                throw new InternalServerErrorException('Error al obtener el enfoque poblacional');
            }
    ...
    async delete(id: number): Promise<EnfoquePoblacionalResponse> {
        try{
            ...
            if (metaError) {
                throw new InternalServerErrorException('Error al verificar si el enfoque poblacional está siendo usado en meta_producto');
            }
            
            if (metaProductos.length > 0) {
                return {
                    status: false,
                    message: 'No se puede eliminar el enfoque poblacional porque está siendo usado en metas de producto',
            ...
            if (deletedError) {
                throw new InternalServerErrorException('Error al eliminar el enfoque poblacional');
            }
