...
export class AreaService {
  ...
  async findOne(id: number): Promise<AreaResponse> {
    try {
      ...
      if (error) {
        throw new InternalServerErrorException(
          'Error al obtener área: ' + error.message,
        );
      }
  ...
  async create(createRequest: AreaRequest): Promise<AreaResponse> {
    try {
      ...
      if (validationError) {
        throw new InternalServerErrorException(
          'Error al validar nombre de área',
        );
      }
      ...
      if (createRequest.telefono && createRequest.telefono.trim().length > 0) {
        ...
        if (telefonoValidationError) {
          throw new InternalServerErrorException(
            'Error al validar teléfono de área',
          );
        }
      ...
      const areaData = {
        ...createRequest,
        modulos:
          Object.keys(modulosValidados).length > 0 ? modulosValidados : null,
      };
  ...
  async update(id: number, createRequest: AreaRequest): Promise<AreaResponse> {
    try {
      ...
      if (checkError) {
        throw new InternalServerErrorException(
          'Error al verificar área: ' + checkError.message,
        );
      }
      ...
      const modulosChanged =
        JSON.stringify(modulosActuales) !== JSON.stringify(modulosValidados);
  ...
  async delete(id: number): Promise<AreaResponse> {
    try {
      ...
      const { data: usuariosArea, error: usuariosError } =
        await this.supabaseService.clientAdmin
          .from('usuario_area')
      ...
      if (usuariosError) {
        throw new InternalServerErrorException(
          'Error al verificar uso del área en usuario_area: ' +
      ...
      if (metasError) {
        throw new InternalServerErrorException(
          'Error al verificar uso del área en meta_producto: ' +
            metasError.message,
        );
      }
