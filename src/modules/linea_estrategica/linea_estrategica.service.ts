...
export class LineaEstrategicaService {
  ...
  async findOne(id: number): Promise<LineaEstrategicaResponse> {
    try {
      ...
      if (error) {
        throw new InternalServerErrorException(
          'Error al buscar línea estratégica: ' + error.message,
        );
      }
  ...
  async create(
    createRequest: LineaEstrategicaRequest,
  ): Promise<LineaEstrategicaResponse> {
    try {
      ...
      if (validationError) {
        throw new InternalServerErrorException(
          'Error al validar nombre de línea estratégica: ' +
      ...
      if (error) {
        throw new InternalServerErrorException(
          'Error al crear línea estratégica: ' + error.message,
        );
      }
  ...
  async update(
    id: number,
    updateRequest: LineaEstrategicaRequest,
  ): Promise<LineaEstrategicaResponse> {
    try {
      ...
      if (checkError) {
        throw new InternalServerErrorException(
          'Error al verificar línea estratégica: ' + checkError.message,
        );
      }
      ...
      if (
        ...
        existingData.plan_departamental ===
          updateRequest.plan_departamental.trim()
      ) {
        return {
          status: false,
      ...
      if (error) {
        throw new InternalServerErrorException(
          'Error al actualizar línea estratégica: ' + error.message,
        );
      }
  ...
  async delete(id: number): Promise<LineaEstrategicaResponse> {
    try {
      ...
      const { data: programasAsociados, error: programasError } =
        await this.supabaseService.clientAdmin
          .from('programa')
          .select('id, nombre')
      ...
      if (programasError) {
        throw new InternalServerErrorException(
          'Error al verificar programas asociados: ' + programasError.message,
        );
      }
      ...
      const { data: metasAsociadas, error: metasError } =
        await this.supabaseService.clientAdmin
          .from('meta_resultado')
          .select('id, nombre')
          .eq('linea_estrategica_id', id)
      ...
      if (metasError) {
        throw new InternalServerErrorException(
          'Error al verificar metas resultado asociadas: ' + metasError.message,
        );
      }
      ...
      if (error) {
        throw new InternalServerErrorException(
          'Error al eliminar línea estratégica: ' + error.message,
        );
      }
