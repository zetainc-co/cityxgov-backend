...
export class UsuariosService {
  ...
  async create(createRequest: UsuarioRequest): Promise<UsuarioResponse> {
    try {
      ...
      const { data: existingArea, error: areaError } =
      ...
      if (areaError) {
        throw new InternalServerErrorException(
          'Error al validar área: ' + areaError.message,
        );
      }
      ...
      const { data: existingRol, error: rolError } =
        await this.supabaseService.clientAdmin
          .from('rol')
          .select('id, nombre')
      ...
      if (rolError) {
        throw new InternalServerErrorException(
          'Error al validar rol: ' + rolError.message,
        );
      }
      ...
      const { data: existingByIdentificacion } =
        await this.supabaseService.clientAdmin
          .from('usuarios')
      ...
      if (userError) {
        ...
        if (userError.code === '23505') {
          if (
            userError.message.includes('profile_identification_key') ||
            userError.message.includes('identificacion')
          ) {
            return {
              status: false,
              message: `El número de identificación ${createRequest.identificacion} ya está registrado en el sistema`,
          ...
          if (
            userError.message.includes('profile_email_key') ||
          ...
          if (
            userError.message.includes('profile_phone_key') ||
            userError.message.includes('telefono')
          ) {
            return {
              status: false,
              message: `El número de teléfono ${createRequest.telefono} ya está registrado en el sistema`,
      ...
      const { data: asignacionData, error: asignacionError } =
        await this.supabaseService.clientAdmin
          .from('usuario_area')
          .insert([
            {
      ...
      if (asignacionError) {
        ...
        throw new InternalServerErrorException(
          'Error al crear asignación: ' + asignacionError.message,
        );
      }
        ...
        asignaciones: [
          {
            ...
            area: Array.isArray(asignacionData.area)
              ? asignacionData.area[0]
              : asignacionData.area,
            rol: Array.isArray(asignacionData.rol)
              ? asignacionData.rol[0]
              : asignacionData.rol,
  ...
  async update(
    id: number,
    updateRequest: UsuarioUpdateRequest,
    ...
    try {
      ...
      if (checkError) {
        throw new InternalServerErrorException(
          'Error al verificar usuario: ' + checkError.message,
        );
      }
      ...
      if (Object.keys(updateData).length > 0) {
        hayCambiosUsuario = Object.keys(updateData).some(
          (key) => existingData[key] !== updateData[key],
        );
      }
      ...
      if (
        updateRequest.area_id !== undefined ||
        updateRequest.rol_id !== undefined
      ) {
        ...
        if (asignacion) {
          if (
            updateRequest.area_id !== undefined &&
            asignacion.area_id !== updateRequest.area_id
          ) {
            hayCambiosAsignacion = true;
          }
          if (
            updateRequest.rol_id !== undefined &&
            asignacion.rol_id !== updateRequest.rol_id
          ) {
            hayCambiosAsignacion = true;
          }
        }
      ...
      if (hayCambiosUsuario && Object.keys(updateData).length > 0) {
        ...
        if (updateError) {
          ...
          if (updateError.code === '23505') {
            if (
              updateError.message.includes('profile_identification_key') ||
              updateError.message.includes('identificacion')
            ) {
              return {
                status: false,
            ...
            if (
              updateError.message.includes('profile_email_key') ||
              updateError.message.includes('correo')
            ) {
              return {
                status: false,
            ...
            if (
              updateError.message.includes('profile_phone_key') ||
              updateError.message.includes('telefono')
            ) {
              return {
                status: false,
          ...
          throw new InternalServerErrorException(
            'Error al actualizar usuario: ' + updateError.message,
          );
        }
      }
      ...
      if (hayCambiosAsignacion && currentAsignacion) {
        ...
        if (updateRequest.area_id !== undefined)
          asignacionUpdate.area_id = updateRequest.area_id;
        if (updateRequest.rol_id !== undefined)
          asignacionUpdate.rol_id = updateRequest.rol_id;

        if (Object.keys(asignacionUpdate).length > 0) {
          const { error: asignacionError } =
            await this.supabaseService.clientAdmin
              .from('usuario_area')
          ...
          if (asignacionError) {
            throw new InternalServerErrorException(
              'Error al actualizar asignación: ' + asignacionError.message,
            );
          }
        }
      }
  ...
  async delete(id: number): Promise<UsuarioResponse> {
    try {
      ...
      if (asignacionError) {
        throw new InternalServerErrorException(
          'Error al eliminar asignaciones: ' + asignacionError.message,
        );
      }
      ...
      if (deleteError) {
        throw new InternalServerErrorException(
          'Error al eliminar usuario: ' + deleteError.message,
        );
      }
  ...
  private async getUsuarioAsignaciones(
    usuarioId: number,
  ...
  async changePassword(
    ...
    try {
      ...
      if (userError) {
        throw new InternalServerErrorException(
          'Error al verificar usuario: ' + userError.message,
        );
      }
  ...
  async createLegacy(createRequest: NuevoUsuario) {
    ...
    throw new BadRequestException(
      'Debe usar el nuevo método create que requiere área y rol',
    );
  }
