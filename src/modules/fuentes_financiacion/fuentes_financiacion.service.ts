...
import {
  ...
  FuentesFinanciacionResponse,
} from './dto/fuentes_financiacion';
...
export class FuentesFinanciacionService {
  ...
  ): Promise<FuentesFinanciacionResponse> {
    try {
      const { data, error } = await this.supabaseService.clientAdmin
        .from('fuentes_financiacion')
  ...
  async delete(id: number): Promise<FuentesFinanciacionResponse> {
    try {
      ...
      const {
        data: programacionFinanciera,
        error: programacionFinancieraError,
      } = await this.supabaseService.clientAdmin
