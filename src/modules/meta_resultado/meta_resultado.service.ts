...
export class MetaResultadoService {
    ...
    try {
      ...
      if (error) {
        throw new InternalServerErrorException(
          'Error al obtener metas de resultado: ' + error.message,
        );
      }
  ...
  async findOne(id: number): Promise<MetaResultadoResponse> {
    try {
      ...
      if (error) {
        throw new InternalServerErrorException(
          'Error al buscar meta de resultado: ' + error.message,
        );
      }
  ...
  async create(
    createRequest: MetaResultadoRequest,
  ): Promise<MetaResultadoResponse> {
    try {
      ...
      const { data: existingMetaResultado, error: metaResultadoError } =
        await this.supabaseService.clientAdmin
          .from('meta_resultado')
          .select('id')
      ...
      if (metaResultadoError) {
        throw new InternalServerErrorException(
          'Error al validar nombre de meta resultado: ' +
            metaResultadoError.message,
        );
      }
      ...
      if (error) {
        throw new InternalServerErrorException(
          'Error al crear meta de resultado: ' + error.message,
        );
      }
  ...
  async update(
    id: number,
    updateRequest: MetaResultadoRequest,
  ): Promise<MetaResultadoResponse> {
    try {
      // 1. Obtener actual
      const { data: current, error: currError } =
        await this.supabaseService.clientAdmin
          .from('meta_resultado')
          .select('*')
          .eq('id', id)
      ...
      if (currError) {
        throw new InternalServerErrorException(
          'Error al verificar meta de resultado: ' + currError.message,
        );
      }
      ...
      const { data: duplicateName, error: nameError } =
        await this.supabaseService.clientAdmin
          .from('meta_resultado')
          .select('id')
      ...
      if (nameError) {
        throw new InternalServerErrorException(
          'Error al validar nombre duplicado: ' + nameError.message,
        );
      }
      ...
      if (error) {
        throw new InternalServerErrorException(
          'Error al actualizar meta de resultado: ' + error.message,
        );
      }
  ...
  async delete(id: number): Promise<MetaResultadoResponse> {
    try {
      ...
      if (existError) {
        throw new InternalServerErrorException(
          'Error al verificar meta de resultado: ' + existError.message,
        );
      }
      ...
      const { data: metasProducto, error: metasError } =
        await this.supabaseService.clientAdmin
      ...
      if (metasError) {
        throw new InternalServerErrorException(
          'Error al verificar uso de la meta de resultado: ' +
            metasError.message,
        );
      }
      ...
      if (metasProducto && metasProducto.length > 0) {
        return {
          status: false,
          message:
            'No se puede eliminar la meta de resultado porque está siendo usada en metas de producto',
      ...
      if (error) {
        throw new InternalServerErrorException(
          'Error al eliminar meta de resultado: ' + error.message,
        );
      }
