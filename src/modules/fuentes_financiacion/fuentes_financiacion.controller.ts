...
import {
  CreateFuentesFinanciacionPipe,
  ValidateIdPipe,
} from './pipes/fuentes_financiacion.pipe';

@Controller('fuentes-financiacion')
@UseGuards(JwtAuthGuard, RolesGuard)
export class FuentesFinanciacionController {
  constructor(
    private readonly fuentesFinanciacionService: FuentesFinanciacionService,
  ) {}
  ...
  @Roles('superadmin', 'admin')
  async create(
    @Body(CreateFuentesFinanciacionPipe)
    createRequest: FuentesFinanciacionRequest,
  ) {
  ...
  async update(
    ...
    @Body(CreateFuentesFinanciacionPipe)
    updateRequest: FuentesFinanciacionRequest,
  ) {
